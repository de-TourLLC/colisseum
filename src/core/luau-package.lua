...
local function random_key(data, seed)
    local state = digest(data, digest(tostring(seed or "") .. tostring(os.time()) .. tostring(os.clock())))
    local key = {}
    for index = 1, 32 do
...
local function marker_prefix(seed)
    local state = digest("names", digest(tostring(seed or "") .. tostring(os.clock())))
    local prefix = "coli_"
    for _ = 1, 6 do
...
local function seal(data, seed, prefix)
    ...
    local mask_seed = (digest(tostring(seed or "") .. tostring(os.clock()) .. tostring(#data)) % (MASK_MOD - 1)) + 1
    local mask = mask_stream(mask_seed, #key)
    local masked = {}
    for index = 1, #key do masked[index] = (key[index] + mask[index]) % 256 end
    local outer_seed = (digest(tostring(seed or "") .. "|L2|" .. tostring(os.clock()) .. tostring(#data)) % (MASK_MOD - 1)) + 1
    local outer_mask = mask_stream(outer_seed, #payload)
    local outer = {}
    for index = 1, #payload do outer[index] = (payload[index] + outer_mask[index]) % 256 end
    local const_key = (digest(tostring(seed or "") .. "|CC|" .. tostring(os.clock())) % 2147483646) + 1
...
if _q(_d..table.concat(_z))~=_h then error("0x3E9D",0) end
return _d
end)()]=]):format(array(masked), mask_seed, array(outer), checksum, outer_seed, c1, c2, c3, c4, const_key)
...
function Package.from_source(source, compiler, fiu, options)
    ...
    local fiu_source, message = read(fiu or "vendor/Fiu/Source.lua")
